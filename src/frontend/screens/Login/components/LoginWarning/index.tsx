...
const LoginWarning = function ({
  ...
  if (warnLoginForStore === 'epic') {
    textContent = t(
      'not_logged_in.epic',
      "You are not logged in with an Epic account in NovaPlay. Don't use the store page to login, click the following button instead:"
    )
    loginPath = epicLoginPath
  } else if (warnLoginForStore === 'gog') {
    textContent = t(
      'not_logged_in.gog',
      "You are not logged in with a GOG account in NovaPlay. Don't use the store page to login, click the following button instead:"
    )
